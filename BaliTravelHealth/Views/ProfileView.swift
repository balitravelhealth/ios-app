...
struct ProfileView: View {
    ...
    @State private var showHealthProfileEdit = false
    @State private var showSignOutConfirmation = false
    @State private var healthProfileService = HealthProfileService.shared
    ...
    var body: some View {
        ...
        .task { await healthProfileService.fetch() }
        .sheet(isPresented: $showEdit) {
            NavigationStack { ProfileEditView() }
                .presentationDetents([.large])
                .presentationDragIndicator(.visible)
        }
        .sheet(isPresented: $showHealthProfileEdit) {
            NavigationStack { HealthProfileEditView() }
                .presentationDetents([.large])
                .presentationDragIndicator(.visible)
        }
    ...
    private var healthProfileSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            let hp = healthProfileService.profile
            VStack(spacing: 0) {
                healthProfileRow(label: "Height",
                                 value: hp?.tinggiCm.map { "\(Int($0)) cm" } ?? "—")
                Divider().padding(.leading, 16)
                healthProfileRow(label: "Weight",
                                 value: hp?.beratKg.map { "\(Int($0)) kg" } ?? "—")
                Divider().padding(.leading, 16)
                healthProfileRow(label: "Blood Type",
                                 value: hp?.golonganDarah?.rawValue ?? "—")
                Divider().padding(.leading, 16)
                healthProfileRow(label: "Allergies",
                                 value: hp?.riwayatAlergi.flatMap { $0.isEmpty ? nil : $0 } ?? "—")
            }
            .background(
                RoundedRectangle(cornerRadius: 18, style: .continuous)
                    .fill(Color(.systemGray5))
    ...
    private func healthProfileRow(label: String, value: String) -> some View {
        HStack {
            Text(label)
                .font(.subheadline)
                .foregroundStyle(Color(.label))
    ...
    private var actionsSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            NavigationLink(destination: AssessmentHistoryView()) {
                HStack {
