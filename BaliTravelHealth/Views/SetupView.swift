...
struct SetupView: View {
    ...
    @FocusState private var emergencyContactFocused: Bool

    private let nameMaxLength = 64
    private let emergencyContactMaxLength = 160
    ...
    private var emergencyContactField: some View {
        CapsuleFieldContainer {
            TextField("Emergency Contact", text: $emergencyContact)
                ...
                .autocorrectionDisabled()
                .focused($emergencyContactFocused)
                .submitLabel(.done)
                .onChange(of: emergencyContact) { _, newValue in
                    if newValue.count > emergencyContactMaxLength {
                        emergencyContact = String(newValue.prefix(emergencyContactMaxLength))
                    }
                }
                .accessibilityLabel("Emergency contact")
                .accessibilityHint("Name, phone number, or other emergency contact text")
        }
    }
