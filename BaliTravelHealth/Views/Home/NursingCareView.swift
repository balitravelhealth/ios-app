...
struct NursingCareView: View {
    ...
    var body: some View {
        Group {
            // Nursing care requires live data — show offline state when disconnected
            // and no cached nurses exist yet.
            if !network.isConnected && service.nurses.isEmpty {
                OfflineFeatureView(
                    featureName: "Nursing Care",
                    suggestion: "Connect to the internet to browse and book nurses."
                )
            } else {
                nursingContent
            }
    ...
    private var nursingContent: some View {
        ZStack {
            mainContent
    ...
    private var nurseGrid: some View {
        ...
        return LazyVGrid(columns: columns, spacing: 14) {
            ForEach(service.nurses) { nurse in
                ...
                .accessibilityValue(nurse.baseRate > 0 ? nurse.formattedFromRate() : "")
