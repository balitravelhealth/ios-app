...
final class ProfileStore {
    ...
    func refreshFromServer() async -> Bool {
        ...
        guard let normalizedDOB = SQLDateFormatter.normalizedString(from: fetched.dateOfBirth),
              let dob = SQLDateFormatter.date(from: normalizedDOB),
        ...
        if let travel = fetched.travel,
           let arrivalDate = SQLDateFormatter.normalizedString(from: travel.arrivalDate),
           let departureDate = SQLDateFormatter.normalizedString(from: travel.departureDate),
           let arrival = SQLDateFormatter.date(from: arrivalDate),
           let departure = SQLDateFormatter.date(from: departureDate) {
